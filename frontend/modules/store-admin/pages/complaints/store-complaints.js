 ...
 * Component  : Store Admin Complaints Overview — Separate Controller
 * File       : store-complaints.js
 ******************************************************************************/
...
const TEMPLATE_URL = 'modules/store-admin/pages/complaints/store-complaints.html';
const CSS_ID       = 'store-complaints-sep-css';
const CSS_URL      = 'modules/store-admin/pages/complaints/store-complaints.css';
...
export default class StoreAdminComplaints {
  ...
  async mount(container) {
    logger.info('StoreAdminComplaints', 'Mounting Store Admin Complaints Overview…');
  ...
  _bindEvents() {
    this.container.querySelector('#btn-sc-refresh')?.addEventListener('click', () => this._loadData());

    const tabs = this.container.querySelectorAll('.sc-filter-tab');
    tabs.forEach(t => {
    ...
    const modal = this.container.querySelector('#sc-action-modal');
    const closeModal = () => { if (modal) modal.style.display = 'none'; };
    this.container.querySelector('#btn-sc-close-modal')?.addEventListener('click', closeModal);
    this.container.querySelector('#btn-sc-cancel-modal')?.addEventListener('click', closeModal);
    modal?.addEventListener('click', e => { if (e.target === modal) closeModal(); });

    this.container.querySelector('#sc-action-form')?.addEventListener('submit', e => this._handleSubmitAction(e));

    // Process Guide Modal Events
    const processBtn = this.container.querySelector('#sc-btn-process-guide');
    const processModal = this.container.querySelector('#sc-process-modal');
    const closeProcessBtn = this.container.querySelector('#btn-sc-close-process-modal');
  ...
  async _loadData() {
    const listEl = this.container.querySelector('#sc-complaints-list');
    if (listEl) listEl.innerHTML = `<div style="padding:20px;text-align:center;color:rgba(255,255,255,0.4);">Loading store complaints table…</div>`;

    const storeId = this.user?.storeId || 1;
    try {
      const res = await apiClient.get(`/api/v1/complaints/overview?targetRole=STORE_ADMIN&storeId=${storeId}`);
      ...
      logger.error('StoreAdminComplaints', 'Failed loading store complaints', err);
      this.complaints = [];
    }
  ...
  _updateKpis() {
    ...
    const elOpen = this.container.querySelector('#sc-kpi-open');
    const elInv  = this.container.querySelector('#sc-kpi-inv');
    const elAct  = this.container.querySelector('#sc-kpi-act');
    const elEsc  = this.container.querySelector('#sc-kpi-esc');
  ...
  _render() {
    ...
    const filtered = this.activeFilter ? this.complaints.filter(c => c.status === this.activeFilter) : this.complaints;
    if (!filtered.length) {
      listEl.innerHTML = `<div style="padding:24px;text-align:center;color:rgba(255,255,255,0.4);font-size:0.83rem;">No store complaints found for this filter.</div>`;
      return;
    }
    ...
    const rows = filtered.map(c => {
      ...
      return `
        <tr style="border-bottom:1px solid rgba(255,255,255,0.06);">
          ...
          <td style="padding:12px;font-size:0.75rem;font-weight:700;color:#c9a46a;">${c.targetRole || 'STORE_ADMIN'}</td>
          ...
          <td style="padding:12px;text-align:right;">
            <button type="button" class="sc-btn sc-btn-secondary btn-sc-respond" data-id="${c.id}" style="padding:5px 10px;font-size:0.75rem;">
              ✏️ Take Action
            </button>
          </td>
        </tr>`;
    ...
    listEl.querySelectorAll('.btn-sc-respond').forEach(btn => {
  ...
  _openModal(item) {
    const modal = this.container.querySelector('#sc-action-modal');
    if (!modal) return;
    this.container.querySelector('#sc-modal-id').value      = item.id;
    this.container.querySelector('#sc-modal-key').value     = item.trackingKey;
    this.container.querySelector('#sc-modal-subject').value = item.subject;
    this.container.querySelector('#sc-modal-status').value  = item.status || 'UNDER_INVESTIGATION';
    this.container.querySelector('#sc-modal-report').value  = item.complianceResponse || '';
    modal.style.display = 'flex';
  }

  async _handleSubmitAction(e) {
    e.preventDefault();
    const id     = parseInt(this.container.querySelector('#sc-modal-id').value, 10);
    const status = this.container.querySelector('#sc-modal-status').value;
    const report = this.container.querySelector('#sc-modal-report').value?.trim();

    const isClosing = ['RESOLVED', 'CLOSED'].includes(status);
    if (isClosing && !report) {
      this.container.querySelector('#sc-modal-report')?.focus();
      notificationStore.warning('Official store investigation resolution report is mandatory to mark as Resolved or Closed.');
      return;
    }
  ...
  destroy() {
    logger.info('StoreAdminComplaints', 'Store Admin Complaints Overview unmounted.');
  }
}
