 ...
 * File              : profile.js
 * Path              : frontend/modules/regional-admin/pages/profile/profile.js
 * Purpose           : Regional Admin user profile workspace component; handles self-profile data loading from GET /api/v1/auth/me, onboarding document status, profile updating via PUT /api/v1/auth/me, custom avatar uploading, and DocumentHubComponent integration.
 * Version           : 1.0.0
 *
 * Description
 * ---------------------------------------------------------------------------
 * Specialized profile page controller for Regional Admin.
 * Capabilities:
 *   - Injects HTML template modules/regional-admin/pages/profile/profile.html and stylesheet profile.css.
 ...
 *   - Renders DocumentHubComponent configured specifically for regionalAdmin role type.
 ******************************************************************************/
...
const TEMPLATE_URL = 'modules/regional-admin/pages/profile/profile.html';

export default class RegionalAdminProfilePage {
  constructor() {
  ...
  async mount(container, lifecycle) {
    logger.info('RegionalAdminProfile', 'Mounting regional admin profile page...');
    this._loadCss();
  ...
  _loadCss() {
    const cssId = 'css-regional-admin-profile';
    if (!document.getElementById(cssId)) {
      const link = document.createElement('link');
      link.id = cssId; link.rel = 'stylesheet';
      link.href = 'modules/regional-admin/pages/profile/profile.css';
  ...
  async loadProfileData() {
    ...
    } catch (e) { logger.error('RegionalAdminProfile', 'Error loading profile:', e); }
  }
  ...
  async loadDocumentData() {
    ...
    } catch (e) { logger.error('RegionalAdminProfile', 'Error loading documents:', e); }
  }
  ...
  render(container) {
    ...
    const regionName = p.regionName || p.region || '';

    const nameEl = container.querySelector('#reg-user-fullname'); if (nameEl) nameEl.textContent = fullName || 'Regional Admin Profile';
    const emailEl = container.querySelector('#reg-user-email'); if (emailEl) emailEl.textContent = email || 'No Email Registered';
    const scopeBadgeEl = container.querySelector('#reg-scope-badge'); if (scopeBadgeEl) scopeBadgeEl.textContent = regionName ? `🌐 ${regionName}` : '🌐 Regional Hub';
    const avatarImg = container.querySelector('#reg-avatar-img');
    const countryBadge = container.querySelector('#reg-country-code-badge');
    ...
    const docsBadge = container.querySelector('#reg-docs-verified-badge');
    if (docsBadge) {
    ...
    const fields = {
      '#input-reg-firstname': p.firstName || '', '#input-reg-lastname': p.lastName || '',
      '#input-reg-email': email, '#input-reg-phone': p.phone || p.phoneNumber || '',
      '#input-reg-empid': p.employeeId || p.employeeCode || '', '#input-reg-designation': p.designation || p.role || '',
      '#input-reg-joined': p.joinedDate || p.hireDate || '', '#input-reg-salary': p.baseSalary || p.basicSalary || '',
      '#input-reg-bankname': p.bankName || '', '#input-reg-bankacc': p.bankAccount || p.bankAccountNumber || '',
      '#input-reg-ifsc': p.ifscCode || p.ifscNumber || '', '#input-reg-branch': p.branchName || p.branchLocation || '',
    };
    Object.entries(fields).forEach(([sel, val]) => { const el = container.querySelector(sel); if (el) el.value = val; });
    const genderSelect = container.querySelector('#input-reg-gender');
    if (genderSelect) genderSelect.value = p.gender || 'Male';

    const docHubBox = container.querySelector('#reg-doc-hub-container');
    if (docHubBox && !this.docHub) {
      this.docHub = new DocumentHubComponent({ roleType: 'regionalAdmin', user: this.user, profile: this.profile, initialDocs: this.docs });
  ...
  bindEvents(container, lifecycle) {
    ...
    const actionsBox = container.querySelector('#reg-personal-actions');
    const fnameInput = container.querySelector('#input-reg-firstname');
    const lnameInput = container.querySelector('#input-reg-lastname');
    const phoneInput = container.querySelector('#input-reg-phone');
    const genderSelect = container.querySelector('#input-reg-gender');
    const bankNameInput = container.querySelector('#input-reg-bankname');
    const bankAccInput = container.querySelector('#input-reg-bankacc');
    const ifscInput = container.querySelector('#input-reg-ifsc');
    const branchInput = container.querySelector('#input-reg-branch');
    const editableInputs = [fnameInput, lnameInput, phoneInput, genderSelect, bankNameInput, bankAccInput, ifscInput, branchInput];
    const lockedInputs = [
      container.querySelector('#input-reg-email'), container.querySelector('#input-reg-empid'),
      container.querySelector('#input-reg-designation'), container.querySelector('#input-reg-joined'), container.querySelector('#input-reg-salary')
    ];
    ...
    const toggleEdit = (editing) => {
      ...
      editableInputs.forEach(input => {
        ...
        const w = input.closest('.reg-form-input-wrap') || input.parentElement;
        if (w) { w.style.background = editing ? 'rgba(59,130,246,0.08)' : 'rgba(24,24,27,0.8)'; w.style.borderColor = editing ? 'rgba(59,130,246,0.4)' : 'rgba(255,255,255,0.1)'; w.style.boxShadow = editing ? '0 0 10px rgba(59,130,246,0.15)' : 'none'; }
      });
    ...
    if (genderSelect) {
      const h = (e) => {
        ...
        const ai = container.querySelector('#reg-avatar-img'); if (ai) ai.src = da;
    ...
    const personalForm = container.querySelector('#form-reg-personal-info');
    if (personalForm) {
      const onSubmit = async (e) => {
        ...
        try {
          ...
          catch (err) { logger.warn('RegionalAdminProfile', 'Backend offline:', err); this.profile = { ...this.profile, ...payload }; }
          const updatedFullName = `${this.profile.firstName || ''} ${this.profile.lastName || ''}`.trim() || 'Regional Admin Profile';
          ...
          const heroNameEl = container.querySelector('#reg-user-fullname'); if (heroNameEl) heroNameEl.textContent = updatedFullName;
          ...
          notificationStore.success('Regional admin profile and banking details saved successfully.');
        } catch (err) { logger.error('RegionalAdminProfile', 'Failed to update profile:', err); }
      };
      personalForm.addEventListener('submit', onSubmit); lifecycle.onCleanup(() => personalForm.removeEventListener('submit', onSubmit));
    }

    // Avatar modal
    const avatarFileInput = container.querySelector('#input-reg-avatar-file');
    const avatarImg = container.querySelector('#reg-avatar-img');
    ...
    if (avatarFileInput) {
      const onFileChange = async (e) => {
        ...
        try { const res = await fetch('/api/upload-avatar', { method: 'POST', headers: { 'Content-Type': file.type || 'image/png', 'X-Role': this.user?.role || 'regionalAdmin' }, body: file }); const data = await res.json(); if (data.success && data.url) updateAvatarUrl(data.url); }
        catch (err) { logger.error('RegionalAdminProfile', 'Avatar upload error:', err); }
      };
