 ...
 * Component  : National Admin Executive Complaints — Separate Controller
 * File       : national-complaints.js
 ******************************************************************************/
...
const TEMPLATE_URL = 'modules/national-admin/pages/complaints/national-complaints.html';
const CSS_ID       = 'national-complaints-sep-css';
const CSS_URL      = 'modules/national-admin/pages/complaints/national-complaints.css';
...
export default class NationalAdminComplaints {
  ...
  async mount(container) {
    logger.info('NationalAdminComplaints', 'Mounting National Admin Complaints Registry…');
  ...
  _bindEvents() {
    this.container.querySelector('#btn-nc-refresh')?.addEventListener('click', () => this._loadData());
    this.container.querySelector('#nc-filter-store')?.addEventListener('change', () => this._render());

    const tabs = this.container.querySelectorAll('.nc-filter-tab');
    tabs.forEach(t => {
    ...
    const modal = this.container.querySelector('#nc-action-modal');
    const closeModal = () => { if (modal) modal.style.display = 'none'; };
    this.container.querySelector('#btn-nc-close-modal')?.addEventListener('click', closeModal);
    this.container.querySelector('#btn-nc-cancel-modal')?.addEventListener('click', closeModal);
    modal?.addEventListener('click', e => { if (e.target === modal) closeModal(); });

    this.container.querySelector('#nc-action-form')?.addEventListener('submit', e => this._handleSubmitAction(e));

    // Process Guide Modal Events
    const processBtn = this.container.querySelector('#nc-btn-process-guide');
    const processModal = this.container.querySelector('#nc-process-modal');
    const closeProcessBtn = this.container.querySelector('#btn-nc-close-process-modal');
  ...
  async _loadData() {
    try {
      const res = await apiClient.get('/api/v1/complaints/overview?targetRole=NATIONAL_ADMIN');
      ...
      logger.error('NationalAdminComplaints', 'Failed loading live complaints', err);
      this.complaints = [];
    }
    this._updateKpis();
    this._render();
  ...
  _updateKpis() {
    ...
    const elOpen = this.container.querySelector('#nc-kpi-open');
    const elInv  = this.container.querySelector('#nc-kpi-inv');
    const elAct  = this.container.querySelector('#nc-kpi-act');
  ...
  _render() {
    const listEl = this.container.querySelector('#nc-complaints-list');
    if (!listEl) return;

    const storeVal  = this.container.querySelector('#nc-filter-store')?.value;
    ...
    if (!filtered.length) {
      listEl.innerHTML = `<div style="padding:24px;text-align:center;color:rgba(255,255,255,0.4);font-size:0.83rem;">No records found for current filters.</div>`;
      return;
    }
    ...
    const rows = filtered.map(c => {
      ...
      return `
        <tr style="border-bottom:1px solid rgba(255,255,255,0.06);">
          ...
          <td style="padding:12px;font-size:0.75rem;font-weight:700;color:#c9a46a;">${c.targetRole || 'NATIONAL_ADMIN'}</td>
          ...
          <td style="padding:12px;text-align:right;">
            <button type="button" class="nc-btn nc-btn-secondary btn-nc-respond" data-id="${c.id}" style="padding:5px 10px;font-size:0.75rem;">
              ✏️ Take Action
            </button>
          </td>
        </tr>`;
    ...
    listEl.querySelectorAll('.btn-nc-respond').forEach(btn => {
  ...
  _openModal(item) {
    const modal = this.container.querySelector('#nc-action-modal');
    if (!modal) return;
    this.container.querySelector('#nc-modal-id').value      = item.id;
    this.container.querySelector('#nc-modal-key').value     = item.trackingKey;
    this.container.querySelector('#nc-modal-subject').value = item.subject;
    this.container.querySelector('#nc-modal-status').value  = item.status || 'UNDER_INVESTIGATION';
    this.container.querySelector('#nc-modal-report').value  = item.complianceResponse || '';
    modal.style.display = 'flex';
  }

  async _handleSubmitAction(e) {
    e.preventDefault();
    const id     = parseInt(this.container.querySelector('#nc-modal-id').value, 10);
    const status = this.container.querySelector('#nc-modal-status').value;
    const report = this.container.querySelector('#nc-modal-report').value?.trim();

    const isClosing = ['RESOLVED', 'CLOSED'].includes(status);
    if (isClosing && !report) {
      notificationStore.warning('Official closing report is mandatory to mark as Resolved or Closed.');
      return;
    }
  ...
  destroy() {
    logger.info('NationalAdminComplaints', 'National Admin Controller unmounted.');
  }
}
