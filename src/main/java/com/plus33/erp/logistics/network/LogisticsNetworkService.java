/******************************************************************************
 * Project           : PLUS33 Coffee ERP
 * Developed By      : Haulo
 * Developed For     : PLUS33 Coffee
 * Developer         : Sivasurya
 *
 * Module            : Logistics Module
 * Package           : com.plus33.erp.logistics.network
 * File              : LogisticsNetworkService.java
 * Purpose           : Business logic service layer for Logistics Module operations
 * Version           : 0.0.1-SNAPSHOT
 *
 * Related Controller: LogisticsNetworkController
 * Related Service   : LogisticsNetworkService
 * Related Repository: LogisticsNetworkRepository
 * Related Entity    : LogisticsNetwork
 * Related DTO       : N/A
 * Related Mapper    : LogisticsNetworkMapper
 * Related DB Table  : logistics_networks
 * Related REST APIs : N/A
 * Depends On        : Platform Module
 * Used By           : LogisticsNetworkController, LogisticsNetworkServiceImpl
 *
 * Description
 * ---------------------------------------------------------------------------
 * Business service for Logistics Module. Implements LogisticsNetworkService. Encapsulates business rules, @Transactional operations, validations, and event publishing.
 ******************************************************************************/
package com.plus33.erp.logistics.network;

import com.plus33.erp.platform.entity.*;
import com.plus33.erp.platform.repository.*;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.stereotype.Service;
import org.springframework.transaction.annotation.Transactional;
import java.math.BigDecimal;

/**
 * <b>PLUS33 Coffee ERP -- Logistics Module</b>
 *
 * <p><b>Class  :</b> {@code LogisticsNetworkService}</p>
 * <p><b>Package:</b> {@code com.plus33.erp.logistics.network}</p>
 * <p><b>Layer  :</b> Business Service: core logic, validation, and @Transactional operations for Logistics Module.</p>
 *
 * <p><b>Service Flow:</b></p>
 * <pre>
 * LogisticsNetworkController
 *   --> LogisticsNetworkService (this)
 *   --> Validate business rules
 *   --> LogisticsNetworkRepository (read/write 'logistics_networks')
 *   --> LogisticsNetworkMapper (Entity to DTO conversion)
 *   --> Publish domain event (analytics refresh)
 *   --> Return DTO response to Controller
 * </pre>
 *
 * <p><b>Database Table   :</b> {@code logistics_networks}</p>
 * <p><b>Module Deps      :</b> Platform</p>
 *
 * @author Sivasurya (Developed for PLUS33 Coffee by Haulo)
 * @version 0.0.1-SNAPSHOT
 */
@Service
public class LogisticsNetworkService {
    @Autowired PlatformLogisticsNodeRepository nodeRepo;
    @Autowired PlatformShippingLaneRepository laneRepo;
    /**
     * Creates a new node and persists it to the database.
     *
     * @param code the code input value
     * @param type the type input value
     * @param lat the lat input value
     * @param lon the lon input value
     * @param region the region input value
     * @param tz the tz input value
     * @param cap the cap input value
     * @return the PlatformLogisticsNode result
     * @throws BusinessException if a business rule is violated
     */
    @Transactional
    public PlatformLogisticsNode addNode(String code, String type, BigDecimal lat, BigDecimal lon, String region, String tz, int cap) {
        PlatformLogisticsNode node = new PlatformLogisticsNode();
        node.setNodeCode(code);
        node.setNodeType(type);
        node.setLatitude(lat);
        node.setLongitude(lon);
        node.setRegion(region);
        node.setTimezone(tz);
        node.setCapacity(cap);
        node.setStatus("ACTIVE");
        return nodeRepo.save(node);
    }

    /**
     * Creates a new lane and persists it to the database.
     *
     * @param src the src input value
     * @param dest the dest input value
     * @param distance the distance input value
     * @param duration the duration input value
     * @param mode the mode input value
     * @return the PlatformShippingLane result
     * @throws BusinessException if a business rule is violated
     */
    @Transactional
    public PlatformShippingLane addLane(Long src, Long dest, BigDecimal distance, int duration, String mode) {
        PlatformShippingLane lane = new PlatformShippingLane();
        lane.setSourceNodeId(src);
        lane.setDestinationNodeId(dest);
        lane.setDistanceKm(distance);
        lane.setExpectedDurationMinutes(duration);
        lane.setTransportMode(mode);
        return laneRepo.save(lane);
    }
}