/******************************************************************************
 * Project           : PLUS33 Coffee ERP
 * Developed By      : Haulo
 * Developed For     : PLUS33 Coffee
 * Developer         : Sivasurya
 *
 * Module            : Hcm Module
 * Package           : com.plus33.erp.hcm.repository
 * File              : CompensationHistoryRepository.java
 * Purpose           : JPA Repository providing database CRUD for Hcm Module entities
 * Version           : 0.0.1-SNAPSHOT
 *
 * Related Controller: CompensationHistoryController
 * Related Service   : CompensationHistoryService, CompensationHistoryServiceImpl
 * Related Repository: CompensationHistoryRepository
 * Related Entity    : CompensationHistory
 * Related DTO       : N/A
 * Related Mapper    : CompensationHistoryMapper
 * Related DB Table  : compensation_historys
 * Related REST APIs : N/A
 * Depends On        : None
 * Used By           : CompensationHistoryService, CompensationHistoryServiceImpl
 *
 * Description
 * ---------------------------------------------------------------------------
 * JPA Repository for Hcm Module against the 'compensation_historys' table. Provides CRUD, Specification-based queries, and paginated results.
 ******************************************************************************/
package com.plus33.erp.hcm.repository;

import com.plus33.erp.hcm.entity.CompensationHistory;
import org.springframework.data.jpa.repository.JpaRepository;
import java.util.List;

/**
 * <b>PLUS33 Coffee ERP -- Hcm Module</b>
 *
 * <p><b>Class  :</b> {@code CompensationHistoryRepository}</p>
 * <p><b>Package:</b> {@code com.plus33.erp.hcm.repository}</p>
 * <p><b>Layer  :</b> JPA Repository: database access for table 'compensation_historys' via Spring Data JPA.</p>
 *
 * <p><b>Database Table   :</b> {@code compensation_historys}</p>
 * <p><b>Module Deps      :</b> Hcm</p>
 *
 * @author Sivasurya (Developed for PLUS33 Coffee by Haulo)
 * @version 0.0.1-SNAPSHOT
 */
public interface CompensationHistoryRepository extends JpaRepository<CompensationHistory, Long> {
    List<CompensationHistory> findByEmployeeId(Long employeeId);
}
